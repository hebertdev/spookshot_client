...
import { getFilesAPI, halloweenBgAPI, uploadFileAPI } from "services/files";
import { prompts } from "../bakground";
import { FileData } from "interfaces/files";

interface HalloweenBgProps {
  handleSetMedia: (media: string[]) => void;
}

export function HalloweenBg({ handleSetMedia }: HalloweenBgProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const get_prompt = prompts.filter((p) => p.value === Number(prompt));

      if (get_prompt.length > 0 && prompt) {
        params.push({
          effect: `gen_background_replace:prompt_an ${get_prompt[0]?.prompt_en}`,
        });
      }
      ...
      const data = await halloweenBgAPI(items[selectedIndex!].id, params);
      ...
      if (imageUrls.length > 0) {
        imageUrls.forEach((url, index) => {
          ...
          img.onerror = () => {
            ...
            showNotification({
              title: "Error",
              message: `Could not transform the image.`,
              color: "red",
            });
          };
        ...
        showNotification({
          title: "Error",
          message: "One or more images were not generated correctly.",
          color: "red",
        });
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (errorGenerating && retryTime > 0) {
      timer = setInterval(() => {
        setRetryTime((prev) => {
          if (prev <= 1000) {
            clearInterval(timer);
  ...
  }, [errorGenerating, retryTime]);

  return (
    <>
      <div className="mt-2">
        <p>
          Create a Halloween-themed image background, featuring an object or
          person prominently in the center.
        </p>
        <Button
          mt={"xs"}
          radius={"lg"}
      ...
      {selectImage && (
        <>
          ...
          <Button
            ...
            loading={loading}
            disabled={errorGenerating || !(aspectRatio && prompt)}
          >
